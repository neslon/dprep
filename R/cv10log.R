...
# por validacion cruzada del clasificador por regresion logistica
# Requiere la funcion multinom de la libreria nnet de B. Ripley
#inputs:
# data: the name of the dataset
# repet: teh number of repetitions
# maxwts: the maximun numger of weigths
#------------------------------------------
require(nnet)
n <- dim(data)[1]
p <- dim(data)[2]
...
datap <- azar[cc,  ]
datat <- azar[ - cc,  ]
tempo =multinom(f1,data=datat,MaxNWts=2500)
tempo1=predict(tempo,datap)
salida[j] <- sum(tempo1 != as.numeric(
datap[, p]))
}
ECV[i] <- sum(salida)/n
}
