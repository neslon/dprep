...
# level: minumum inconsistency level
# Edgar Acuna, 2003
# **************************************************************
n <- dim(data)[1]
f <- dim(data)[2] - 1
...
varia <- f + 1
for(k in 1:f) {
#initializing the classification rate
rate <- rep(1, f)
if(k > 1) {
varia <- c(where, varia)
}
...
#print(prov)
where <- order(rate)[1]
#cat("The entering feature\n")
#print(where)
#correct classfication rate of the entering feature
output[k] <- rate[where]
indic[where] <- 1
#Avoiding ties
if(k > 1) {
...
list(varselec = which1, inconsis = output[1:
length(which1)])
}
