"starcoord" <-
function (data, main=NULL, class=FALSE, outliers,vars=0, scale=1,
...
stop ("data must be a mtrix or a data frame !")
if (!is.numeric(data))
...
plot(0,type="n", xlim=xlim, ylim=ylim, main=main, asp=1,xlab=0)
lab=c(1:nbcol)
for (i in 1:nbcol) {
arrows(o.loc[1], o.loc[2], axcord[1,i],
axcord[2,i],length=0.05, lty=5)
if (i < nbcol/2)
text(axcord[1,i],axcord[2,i], lab[i],pos=3,cex=0.7)
else
text(axcord[1,i],axcord[2,i], lab[i],pos=1,cex=0.7)
}
if (class==FALSE)
points(data[1,],data[2,], pch=par("pch"), type="p",
col="red")
else {
pointcolor=c("blue","red","green","black","purple","brown",
"gray","yellow","light green")
palette(pointcolor)
nombres=outliers
text(data[1,outliers],data[2,outliers],labels=nombres,cex=.7)
data = rbind(data, classinf)
for (i in as.numeric(levels(factor(classinf))))
points(data[1,data[3,]==i],data[2,data[3,]==i],col=i)
}
}
